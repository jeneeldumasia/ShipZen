...
- **Kyverno PolicyExceptions**: When writing a `PolicyException` for Kyverno to bypass rules on Pod-controllers (DaemonSets, Deployments), Kyverno evaluates the auto-generated rules (which start with `autogen-`). However, in the `PolicyException` manifest, you **MUST** specify the original rule name (e.g. `host-namespaces`), **NOT** the auto-generated name (`autogen-host-namespaces`).
