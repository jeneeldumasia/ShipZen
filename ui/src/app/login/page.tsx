...
export default function LoginPage() {
  return (
    <div className="fixed inset-0 flex items-center justify-center overflow-hidden bg-canvas-bg z-0">
      {/* Premium CSS Gradient Background instead of image */}
      <div className="absolute inset-0 z-0 bg-canvas-bg">
      ...
      {/* Glassmorphism Card */}
      <div className="relative z-10 w-full max-w-[420px] rounded-2xl border border-canvas-border bg-canvas-bg/80 p-10 text-center shadow-2xl backdrop-blur-xl">
        ...
        <h1 className="mb-2 text-3xl font-extrabold tracking-tight text-text-primary drop-shadow-md">
          ShipZen
        </h1>
        ...
        >
          <button
            type="submit"
            className="group relative flex w-full items-center justify-center overflow-hidden rounded-xl bg-canvas-border/50 px-4 py-3 text-sm font-semibold text-text-primary transition-all duration-300 hover:bg-canvas-border hover:shadow-[0_0_20px_rgba(139,92,246,0.3)] active:scale-[0.98]"
          >
            {/* Subtle highlight effect on hover */}
            <span className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-text-primary/10 to-transparent transition-transform duration-500 ease-out group-hover:translate-x-full" />
